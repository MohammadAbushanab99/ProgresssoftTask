...
public class Writer<Path> extends NormalizerImpl {

    public void writeToFile(Vector<BigDecimal> Final_Results, Vector<String> Rows_to_Write, int index, String column_name, Path des) {
            ...
            for (int i = 0; i < Final_Results.size(); i++) {
                ...
                for (int j = 0; j < Row.length; j++) {
                    ...
                    if (j == index && j != Row.length - 1) {
                        ...
                        Final_row.append(Final_Results.get(i).toString()).append(",");

                    } else if (j == index && j == Row.length - 1) {

                        Final_row.append(Row[j]).append(",");

                        Final_row.append(Final_Results.get(i).toString());
