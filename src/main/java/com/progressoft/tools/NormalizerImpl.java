...
public class NormalizerImpl implements Normalizer {
    ...
    ScoringSummeryImpl scoringSummary = new ScoringSummeryImpl();//object from Imp_ScoringSummery
    ...
    public ScoringSummary zscore(Path csvPath, Path destPath, String colToStandardize) {
        ...
        try {
            ...
            Results = reader.readFile(csvPath, colToStandardize);//It sends path and column to Reader class
        ...
        calculateScoringSummery(Results, Size);
        ...
        writer.writeToFile(Results, Lines, index_of_col, equation_type, destPath);//Send Results,rows,index of (colToStandardize),equation type and destination path to Writer class
    ...
    public ScoringSummary minMaxScaling(Path csvPath, Path destPath, String colToNormalize) {
        ...
        try {
            ...
            Results = reader.readFile(csvPath, colToNormalize);//It sends path and column to Reader class
        ...
        writer.writeToFile(Results, Lines, index_of_col, Equation_type, destPath);//Send Results,rows,index of (colToNormalize),equation type and destination path to Writer class
    ...
    public void calculateMean(Vector<BigDecimal> Numbers, BigDecimal Size) {
    ...
    public void calculateVariance(Vector<BigDecimal> Numbers, BigDecimal Size) {
    ...
    public void calculateMedian(Vector<BigDecimal> Numbers) {
    ...
    public void calculateMinMax(Vector<BigDecimal> Numbers) {
    ...
    private void calculateScoringSummery(Vector<BigDecimal> Results, BigDecimal Size) {
        calculateMean(Results, Size);//Send result and number of people to calculate mean

        calculateVariance(Results, Size);//Send result and number of people to calculate variance

        calculateStandDiv();//calculate variance

        calculateMedian(Results);//Send result to calculate median

        calculateMinMax(Results);//Send result to calculate min and max
    }
